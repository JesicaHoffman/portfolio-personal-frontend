...
export class DatosPersonalesComponent implements OnInit {
  ...
  modificarDatos(){
   ...
   onEnviarDatos(event:Event){
    ...
    this.datosPersonales[0].fechaNacimiento = this.formDatosPer.get('fechaNacimiento')?.value;
    ...
    this.datosPersonales[0].nacionalidad = this.formDatosPer.get('nacionalidad')?.value;
    this.datosPersonales[0].linkedin = this.formDatosPer.get('linkedin')?.value;
     this.generalService.modifyPersona(this.datosPersonales[0]).subscribe(data =>{
...
eliminar(event:Event){
  event.preventDefault;
  this.generalService.deletePersona(this.datosPersonales[0].id).subscribe(data => {
    this.nombre = ''
    this.apellido = ''
